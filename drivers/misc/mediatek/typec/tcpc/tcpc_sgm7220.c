...
static void process_interrupt_register(struct sgm7220_chip *info)
{
	u8 val, tmp, reg_val;
	int ret;
	struct tcpc_device *tcpc_dev = info->tcpc;
	...
	ret = sgm7220_read_reg(info->client, REG_MOD, &tmp);
	if (ret < 0) {
		pr_err("%s err\n", __func__);
		return;
	}
	pr_info("%s 0x%x = 0x%x, 0x%x = 0x%x\n", __func__, REG_MOD, tmp, REG_INT, reg_val);

	/* check cable dir */
	tmp = reg_val;
	val = ((tmp & INT_CABLE_DIR) >> INT_CABLE_DIR_SHIFT);
	info->type_c_param.cable_dir = val;
	tcpc_dev->typec_polarity = info->type_c_param.cable_dir;
	pr_info("%s sgm7220_typec_polarity =%d \n", __func__,tcpc_dev->typec_polarity);
}
...
static int sgm7220_tcpcdev_init(struct sgm7220_chip *chip, struct device *dev)
{
	...
	sgm7220_update_reg(chip->client, REG_INT,
			0x01 << INT_DRP_DUTY_CYCLE_SHIFT, INT_DRP_DUTY_CYCLE);

	sgm7220_read_reg(chip->client, REG_INT, &value);
	pr_err("%s 0x%x = 0x%x\n", __func__, REG_INT, value);
...
/*
static int sgm7220_gpio_init(struct sgm7220_chip *chip, struct device *dev)
{
	struct device_node *np = dev->of_node;
	...
	chip->power_gpio = of_get_named_gpio(np, "sgm7220,power_gpio", 0);
    if(chip->power_gpio < 0){
    	pr_info("%s: no chip->power_gpio\n", __func__);
        return -EINVAL;
    }

	gpio_request(chip->power_gpio,"sgm7220,power_gpio");
	gpio_direction_output(chip->power_gpio, 1);
	gpio_set_value(chip->power_gpio, 1);
	return 0;
}
*/
				...
				const struct i2c_device_id *id)
{
	...
	//sgm7220_gpio_init(chip, &client->dev);
